...
<body class="">
	...
	<jsp:include page="../include/menues.jsp">
		<jsp:param value="/categorys/1" name="menu"/>
	</jsp:include>
	<div class="content">

	<ul class="breadcrumb">
		<li><a href="<c:url value='/user/welcome.htm'/>">Home</a> <span class="divider">/</span></li>
		<li class="active">字典分类列表</li>
	</ul>
	
		<div class="container-fluid">
			<div class="row-fluid">
			<div class="btn-toolbar">
				<a href="<c:url value="/category/new"/>" class="btn btn-primary">
					<i class="icon-plus"></i>添加字典分类
				</a>
			</div>
			<div class="well">
				...
				<table class="table">
					...
					<tbody>
					<c:forEach items="${dictionaryCategorys}" var="dictionaryCategory" varStatus="status">
						<tr <c:if test="${status.index % 2 == 0 }">class="info"</c:if>>
							<td>${(status.index + 1) + (pages.currentPage - 1) * pages.pageRecord }</td>
							<td>${dictionaryCategory.categoryCode }</td>
							<td>
								<c:choose> 
							...
							<td>${dictionaryCategory.categoryDesc }</td>
							<td>
							    <a href="<c:url value="/category/${dictionaryCategory.id }/edit/${pages.currentPage }"/>" title="编辑修改" class="icon-pencil"></a>
							    <a href="#myModal${dictionaryCategory.id}" role="button" data-toggle="modal" title="删除"><i class="icon-remove"></i></a>
	             					<div class="modal small hide fade" id="myModal${dictionaryCategory.id}" tabindex="-1"
									...
									<div class="modal-footer">
		              					<form id="deleteForm${dictionaryCategory.id}" method="post" action="<c:url value="/category/${dictionaryCategory.id}/delete"/>">
			...
			<div class="pagination">
				<c:if test="${pages.totalPage > 0}">
					...
					<c:if test="${pages.currentPage > 1}">
						<li><a href="<c:url value="/categorys/${pages.prePage }"/>">Prev</a></li>
					</c:if>
					<c:forEach begin="${pages.pageNumStart }" end="${pages.pageNumEnd }" varStatus="status">
						<li <c:if test="${status.index == pages.currentPage }">class="active"</c:if>>
							<a href="<c:url value="/categorys/${status.index }"/>">
								${status.index }
							</a>
						</li>
					</c:forEach>
					<c:if test="${pages.currentPage< pages.totalPage}">
						<li><a href="<c:url value="/categorys/${pages.nextPage }"/>">Next</a></li>
					</c:if>
