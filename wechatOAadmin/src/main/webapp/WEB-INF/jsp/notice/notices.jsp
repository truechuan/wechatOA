			...
			<div class="btn-toolbar">
				...
				区县：&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
					       <c:forEach items="${areas }" var="item" varStatus="status" >
								...
								<input type="radio" name="radio" onclick="javascript:window.location.href='<c:url value="/notices/1/${item.handleOrgId }"/>'"/>${item.handleorgName }</label>
							</c:forEach> 
			   <!-- 前台区县循环End -->
					<table class="table">
						<thead>
				...
				<c:if test="${pages.totalPage > 0}"> 
					...
					<c:if test="${pages.currentPage > 1}">
						<li><a href="<c:url value="/notices/${pages.prePage }/1"/>">Prev</a></li>
					</c:if>
