...
<!-- 								<td><a href="${item.idcardImgUrl2 }" target="_blank">背面照</a></td> -->
								<td>${item.no_promise_num }</td>
								<td>${item.checked_cancel_num }</td>
								<td>${item.stoptime_start }</td>
								<td>${item.stoptime_end }</td>
								<td>
								...
								<td><c:if test="${item.stoptime_status eq 0}"><B style="color: green;">未封</B></c:if>
									<c:if test="${item.stoptime_status eq 1}"><B style="color: red;">已封</B></c:if>
								</td>
								<td>${item.createTime }</td>
								<!--2017-04-12 add start---->
								<td><a href="<c:url value="/unLockedUser/${item.id}/${pages.currentPage }"/>">解封<br/></a></td>
